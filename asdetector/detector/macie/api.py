...
_macie_lib = "libMACIE.so"

if sys.platform.startswith('win32'):
    _macie_lib = 'MACIE.dll'

MACIE = ct.CDLL(_macie_lib)
...
for _attr in MACIE_ATTRS:
    assert hasattr(MACIE, _attr)
...
def error():
    ...
    """
    MACIE.MACIE_Error.restype = ct.c_char_p
    error_msg = MACIE.MACIE_Error()
...
def lib_version():
    ...
    """
    MACIE.MACIE_LibVersion.restype = ct.c_float
    return MACIE.MACIE_LibVersion()
...
def init():
    ...
    """
    MACIE.MACIE_Init.restype = Status
    handle_status(MACIE.MACIE_Init())
...
def free():
    ...
    """
    MACIE.MACIE_Free.restype = Status
    handle_status(MACIE.MACIE_Free())
...
def check_interfaces(gige_command_port=0, ip_address_list=None):
    ...
    if ip_address_list is None:
        p_ip_address_list = None
        n_ip_address = 0
        p_ip_address_type = ct.c_void_p
    else:
        p_ip_address_type = ct.c_ubyte * 4
        ip_address_list = [p_ip_address_type(*ip) for ip in ip_address_list]
        n_ip_address, p_ip_address_list = list_to_array_pointer(ip_address_list, p_ip_address_type)
        p_ip_address_type = ct.POINTER(p_ip_address_type)

    MACIE.MACIE_CheckInterfaces.argtypes = [  # [gigeCommandPort, pIpAddrList, nIpAddr, numCards, pCardInfo]
        ct.c_ushort, p_ip_address_type, ct.c_ushort,
        ct.POINTER(ct.c_ushort), ct.POINTER(ct.POINTER(CardInfo))
    ]
    MACIE.MACIE_CheckInterfaces.restype = Status
    num_cards = ct.c_ushort(0)
    macie_card_info_pointer = ct.POINTER(CardInfo)()
    status = MACIE.MACIE_CheckInterfaces(
        gige_command_port, p_ip_address_list, n_ip_address, ct.byref(num_cards),
        ct.byref(macie_card_info_pointer)
    )
    handle_status(status)
    return [macie_card_info_pointer[i] for i in range(num_cards.value)]
...
def set_gige_timeout(timeout):
    ...
    """
    MACIE.MACIE_SetGigeTimeout.argtypes = [ct.c_ushort]  # [timeout]
    MACIE.MACIE_SetGigeTimeout.restype = Status
    handle_status(MACIE.MACIE_SetGigeTimeout(timeout))
...
def get_handle(macie_serial_number, connection):
    ...
    """
    MACIE.MACIE_GetHandle.argtypes = [ct.c_ushort, ct.c_int]  # [MACIESerialNumber, connection]
    MACIE.MACIE_GetHandle.restype = ct.c_ulong
    return MACIE.MACIE_GetHandle(macie_serial_number, connection)
...
def get_available_macies(handle):
    ...
    """
    MACIE.MACIE_GetAvailableMACIEs.argtypes = [ct.c_ulong]  # [handle]
    MACIE.MACIE_GetAvailableMACIEs.restype = ct.c_ubyte
    return MACIE.MACIE_GetAvailableMACIEs(handle)
...
def get_available_asics(handle, asic_type=0):
    ...
    MACIESHARED_EXPORT unsigned char MACIE_GetAvailableASICs( unsigned long handle, int asicType );
    """
    MACIE.MACIE_GetAvailableASICs.argtypes = [ct.c_ulong, ct.c_int]  # handle, asicType
    MACIE.MACIE_GetAvailableASICs.restype = ct.c_ubyte
    return MACIE.MACIE_GetAvailableASICs(handle, asic_type)
...
def read_macie_register(handle, select_macies, address):
    ...
    """
    value = ct.c_uint(0)
    MACIE.MACIE_ReadMACIEReg.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.POINTER(ct.c_uint)]
    MACIE.MACIE_ReadMACIEReg.restype = Status
    handle_status(MACIE.MACIE_ReadMACIEReg(handle, select_macies, address, ct.byref(value)))
    return value.value
...
def write_macie_register(handle, select_macies, address, value):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_WriteMACIEReg( unsigned long  handle,
                                                         ...
                                                         unsigned int   value );
    """
    MACIE.MACIE_WriteMACIEReg.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.c_uint]
    MACIE.MACIE_WriteMACIEReg.restype = Status
    handle_status(MACIE.MACIE_WriteMACIEReg(handle, select_macies, address, value))
...
def write_macie_block(handle, select_macies, starting_address, values):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_WriteMACIEBlock( unsigned long  handle,
                                                           ...
                                                           int            arrSize );
    """
    MACIE.MACIE_WriteMACIEBlock.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.POINTER(ct.POINTER(ct.c_ushort)), ct.c_int
    ]
    MACIE.MACIE_WriteMACIEBlock.restype = Status
    value_array = (ct.c_ushort * len(values))(*values)
    handle_status(
        MACIE.MACIE_WriteMACIEBlock(handle, select_macies, starting_address, ct.pointer(value_array), len(values))
    )
...
def read_macie_block(handle, select_macies, starting_address, num_read_registers):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_ReadMACIEBlock( unsigned long  handle,
                                                      ...
                                                      int            arrSize );
    """
    MACIE.MACIE_ReadMACIEBlock.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.POINTER(ct.c_ushort*num_read_registers), ct.c_int
    ]
    MACIE.MACIE_ReadMACIEBlock.restype = Status
    value_array = (ct.c_ushort * num_read_registers)()
    handle_status(
        MACIE.MACIE_ReadMACIEBlock(
            handle, select_macies, starting_address, ct.byref(value_array), num_read_registers
        )
    )
    return [value_array[i] for i in range(num_read_registers)]
...
def load_macie_firmware(handle, select_macies, firmware_slot=True):
    ...
    """
    MACIE.MACIE_loadMACIEFirmware.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_bool, ct.POINTER(ct.c_uint)]
    MACIE.MACIE_loadMACIEFirmware.restype = Status
    firmware_result = ct.c_uint(0)
    handle_status(MACIE.MACIE_loadMACIEFirmware(handle, select_macies, firmware_slot, ct.byref(firmware_result)))
    return firmware_result.value
...
def download_macie_file(handle, select_macies, register_file):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_DownloadMACIEFile( unsigned long handle,
                                                             ...
                                                             const char*   regFile );
    """
    MACIE.MACIE_DownloadMACIEFile.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_char_p]
    MACIE.MACIE_DownloadMACIEFile.restype = Status
    handle_status(MACIE.MACIE_DownloadMACIEFile(handle, select_macies, register_file.encode('utf-8')))
...
def get_acadia_address_increment(handle, select_macies):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_GetAcadiaAddressIncrement( unsigned long handle,
                                                                     ...
                                                                     bool*       bAutoAddrInc);
    """
    MACIE.MACIE_GetAcadiaAddressIncrement.argtype = [ct.c_ulong, ct.c_ubyte, ct.POINTER(ct.c_bool)]
    MACIE.MACIE_GetAcadiaAddressIncrement.restype = Status
    auto_address_increment = ct.c_bool(True)
    MACIE.MACIE_GetAcadiaAddressIncrement(handle, select_macies, ct.byref(auto_address_increment))
    return auto_address_increment.value
...
def set_acadia_address_increment(handle, select_macies, auto_address_increment):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_SetAcadiaAddressIncrement( unsigned long handle,
                                                                     ...
                                                                     bool       bAutoAddrInc);
    """
    MACIE.MACIE_SetAcadiaAddressIncrement.argtype = [ct.c_ulong, ct.c_ubyte, ct.c_bool]
    MACIE.MACIE_SetAcadiaAddressIncrement.restype = Status
    handle_status(MACIE.MACIE_SetAcadiaAddressIncrement(handle, select_macies, auto_address_increment))
...
def write_asic_register(handle, select_asics, address, value, b_option):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_WriteASICReg( unsigned long  handle,
                                                        ...
                                                        bool           bOption);
    """
    MACIE.MACIE_WriteASICReg.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.c_uint, ct.c_bool]
    MACIE.MACIE_WriteASICReg.restype = Status
    handle_status(MACIE.MACIE_WriteASICReg(handle, select_asics, address, value, b_option))
...
def read_asic_register(handle, select_asics, address, b_24_bit, b_option):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_ReadASICReg( unsigned long  handle,
                                                       ...
                                                       bool           bOption );
    """
    MACIE.MACIE_ReadASICReg.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.POINTER(ct.c_uint), ct.c_bool, ct.c_bool
    ]
    MACIE.MACIE_ReadASICReg.restype = Status
    value = ct.c_uint(0)
    handle_status(MACIE.MACIE_ReadASICReg(handle, select_asics, address, ct.byref(value), b_24_bit, b_option))
    return value.value
...
def write_asic_block(handle, select_asics, starting_address, values, b_option):
    ...
    MACIE.MACIE_WriteASICBlock.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.c_uint * n_registers, ct.c_int, ct.c_bool
    ]
    MACIE.MACIE_WriteASICBlock.restype = Status
    value_array = (ct.c_ushort * n_registers)(*values)
    handle_status(
        MACIE.MACIE_WriteASICBlock(
            handle, select_asics, starting_address, ct.pointer(value_array), len(values), b_option
        )
    )
...
def read_asic_block(handle, select_asics, starting_address, num_read_registers, b_24_bit, b_option):
    ...
    MACIESHARED_EXPORT MACIE_STATUS  MACIE_ReadASICBlock( unsigned long  handle,
                                                          ...
                                                          bool           bOption );
    """
    MACIE.MACIE_ReadASICBlock.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.POINTER(ct.c_uint*num_read_registers), ct.c_int, ct.c_bool, ct.c_bool
    ]
    MACIE.MACIE_ReadASICBlock.restype = Status
    value_array = (ct.c_uint*num_read_registers)()
    handle_status(
        MACIE.MACIE_ReadASICBlock(
            handle, select_asics, starting_address, ct.pointer(value_array), num_read_registers, b_24_bit, b_option
        )
    )
    return [value_array[i] for i in range(num_read_registers)]
...
def download_asic_file(handle, select_asics, register_file, b_option):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_DownloadASICFile( unsigned long  handle,
                                                            ...
                                                            bool           bOption );
    """
    MACIE.MACIE_DownloadASICFile.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_char_p, ct.c_bool]
    MACIE.MACIE_DownloadASICFile.restype = Status
    handle_status(MACIE.MACIE_DownloadASICFile(handle, select_asics, register_file.encode('utf-8'), b_option))
...
def close_port(handle):
    ...
    """
    MACIE.MACIE_ClosePort.argtypes = [ct.c_ulong]
    MACIE.MACIE_ClosePort.restype = Status
    handle_status(MACIE.MACIE_ClosePort(handle))
...
def reset_error_counters(handle, select_macies):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_ResetErrorCounters( unsigned long handle,
                                                              unsigned char slctMACIEs );
    """
    MACIE.MACIE_ResetErrorCounters.argtypes = [ct.c_ulong, ct.c_ubyte]
    MACIE.MACIE_ResetErrorCounters.restype = Status
    handle_status(MACIE.MACIE_ResetErrorCounters(handle, select_macies))
...
def set_macie_phase_shift(handle, select_macies, clock_phase):
    ...
    """
    MACIE.MACIE_SetMACIEPhaseShift.argtypes = [ct.c_long, ct.c_ubyte, ct.c_short]
    MACIE.MACIE_SetMACIEPhaseShift.restype = Status
    handle_status(MACIE.MACIE_SetMACIEPhaseShift(handle, select_macies, clock_phase))
...
def get_macie_phase_shift(handle, select_macies):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_GetMACIEPhaseShift( unsigned long handle,
                                                              ...
                                                              unsigned short *clkPhase );
    """
    MACIE.MACIE_GetMACIEPhaseShift.argtypes = [ct.c_long, ct.c_ubyte, ct.POINTER(ct.c_short)]
    MACIE.MACIE_GetMACIEPhaseShift.restype = Status
    clock_phase = ct.c_short(0)
    handle_status(MACIE.MACIE_GetMACIEPhaseShift(handle, select_macies, ct.byref(clock_phase)))
    return clock_phase.value
...
def download_load_file(handle, select_macies, select_asics, register_file, b_option):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_DownloadLoadfile( unsigned long  handle,
                                                            ...
                                                            bool           bOption );
    """
    MACIE.MACIE_DownloadLoadfile.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_ubyte, ct.c_char_p, ct.c_bool]
    MACIE.MACIE_DownloadLoadfile.restype = Status
    handle_status(
        MACIE.MACIE_DownloadLoadfile(handle, select_macies, select_asics, register_file.encode('utf-8'), b_option)
    )
...
def get_error_counters(handle, select_macies):
    ...
    """
    MACIE.MACIE_GetErrorCounters.argtypes = [ct.c_ulong, ct.c_ubyte, ct.POINTER(ct.c_ushort)]
    MACIE.MACIE_GetErrorCounters.restype = Status
    error_counters = ct.c_ushort(0)
    handle_status(MACIE.MACIE_GetErrorCounters(handle, select_macies, ct.byref(error_counters)))
    return error_counters.value  # TODO: create an error counter parser
...
def configure_camlink_interface(handle, select_macies, mode, dcf_file, timeout, frame_x, frame_y):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_ConfigureCamLinkInterface( unsigned long handle,
                                                                     ...
                                                                     short          *nBuffers );
    """
    MACIE.MACIE_ConfigureCamLinkInterface.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_short, ct.c_char_p, ct.c_ushort, ct.c_ushort, ct.c_ushort, ct.POINTER(ct.c_short)
    ]
    MACIE.MACIE_ConfigureCamLinkInterface.restype = Status
    buffer_size = ct.c_short(0)
    handle_status(
        MACIE.MACIE_ConfigureCamLinkInterface(
            handle, select_macies, mode, dcf_file.encode('utf-8'), int(timeout*10), frame_x, frame_y,
            ct.byref(buffer_size)
        )
    )
...
def configure_gige_science_interface(handle, select_macies, mode, frame_size, remote_port):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_ConfigureGigeScienceInterface( unsigned long handle,
                                                                         ...
                                                                         int            *bufSize );
    """
    MACIE.MACIE_ConfigureGigeScienceInterface.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.c_int, ct.c_ushort, ct.POINTER(ct.c_int)
    ]
    MACIE.MACIE_ConfigureGigeScienceInterface.restype = Status
    buffer_size = ct.c_int(0)
    handle_status(
        MACIE.MACIE_ConfigureGigeScienceInterface(
            handle, select_macies, mode, frame_size, remote_port, ct.byref(buffer_size)
        )
    )
    return buffer_size.value
...
def configure_usb_science_interface(handle, select_macies, mode, frame_size, n_buffers):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_ConfigureUSBScienceInterface( unsigned long handle,
                                                                        ...
                                                                        short         nBuffers );
    """
    MACIE.MACIE_ConfigureUSBScienceInterface.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_ushort, ct.c_int, ct.c_short]
    MACIE.MACIE_ConfigureUSBScienceInterface.restype = Status
    handle_status(MACIE.MACIE_ConfigureUSBScienceInterface(handle, select_macies, mode, frame_size, n_buffers))
...
def available_science_data(handle):
    ...
    """
    MACIE.MACIE_AvailableScienceData.restype = ct.c_ulong
    MACIE.MACIE_AvailableScienceData.argtypes = [ct.c_ulong]
    return MACIE.MACIE_AvailableScienceData(handle)
...
def available_science_frames(handle):
    ...
    """
    MACIE.MACIE_AvailableScienceFrames.restype = ct.c_ulong
    MACIE.MACIE_AvailableScienceFrames.argtypes = [ct.c_ulong]
    return MACIE.MACIE_AvailableScienceFrames(handle)
...
def read_gige_science_frame(handle, timeout, frame_size):
    ...
    MACIESHARED_EXPORT unsigned short* MACIE_ReadGigeScienceFrame( unsigned long handle,
                                                                   unsigned short timeout );
    """
    MACIE.MACIE_ReadGigeScienceFrame.argtypes = [ct.c_ulong, ct.c_ushort]
    # MACIE.MACIE_ReadGigeScienceFrame.restype = ct.c_ushort * frame_size
    MACIE.MACIE_ReadGigeScienceFrame.restype = ct.POINTER(ct.c_ushort)
    image_array = MACIE.MACIE_ReadGigeScienceFrame(handle, timeout)
    # return image_array
    return handle_image_array(image_array, frame_size)
...
def read_usb_science_frame(handle, timeout, frame_size):
    ...
    MACIESHARED_EXPORT unsigned short* MACIE_ReadUSBScienceFrame( unsigned long handle,
                                                                  unsigned short timeout );
    """
    MACIE.MACIE_ReadUSBScienceFrame.argtypes = [ct.c_ulong, ct.c_ushort]
    MACIE.MACIE_ReadUSBScienceFrame.restype = ct.POINTER(ct.c_ushort)
    image_array = MACIE.MACIE_ReadUSBScienceFrame(handle, timeout)
    return handle_image_array(image_array, frame_size)
...
def read_camlink_science_frame(handle, timeout, frame_size, tif_file_name=''):
    ...
    MACIESHARED_EXPORT unsigned short* MACIE_ReadCamlinkScienceFrame( unsigned long handle,
                                                                  ...
                                                                  unsigned short timeout );
    """
    MACIE.MACIE_ReadCamlinkScienceFrame.argtypes = [ct.c_ulong, ct.c_char_p, ct.c_ushort]
    MACIE.MACIE_ReadCamlinkScienceFrame.restype = ct.POINTER(ct.c_ushort)
    image_array = MACIE.MACIE_ReadCamlinkScienceFrame(handle, tif_file_name.encode('utf-8'), timeout)
    return handle_image_array(image_array, frame_size)
...
def write_fits_file(file_name, frame_x, frame_y, frame_data, headers):
    ...
    """
    MACIE.MACIE_WriteFitsFile.argtypes = [
        ct.c_char_p, ct.c_ushort, ct.c_ushort, ct.POINTER(ct.c_ushort), ct.c_ushort, ct.POINTER(FitsHeader)
    ]
    MACIE.MACIE_WriteFitsFile.restype = Status
    header = (FitsHeader * len(headers))(*headers)
    data = (ct.c_ushort * len(frame_data))(frame_data)
    handle_status(
        MACIE.MACIE_WriteFitsFile(file_name.encode('utf-8'), frame_x, frame_y, data, len(headers), header)
    )
...
def read_gige_science_data(handle, timeout, n_science_words_read):
    ...
    MACIESHARED_EXPORT int MACIE_ReadGigeScienceData( unsigned long handle,
                                                      ...
                                                      unsigned short *pData );
    """
    MACIE.MACIE_ReadGigeScienceData.argtypes = [
        ct.c_ulong, ct.c_ushort, ct.c_long, ct.POINTER(ct.c_ushort * n_science_words_read)
    ]
    MACIE.MACIE_ReadGigeScienceData.restype = ct.c_int
    science_words = (ct.c_ushort * n_science_words_read)()
    n_science_words_output = MACIE.MACIE_ReadGigeScienceData(
        handle, timeout, n_science_words_read,  ct.byref(science_words)
    )
    print(n_science_words_output)
    return handle_image_array(science_words, n_science_words_output)
...
def read_usb_science_data(handle, timeout, n_science_words_read):
    ...
    MACIESHARED_EXPORT int MACIE_ReadUSBScienceData(  unsigned long handle,
                                                  ...
                                                  unsigned short *pData );
    """
    MACIE.MACIE_ReadUSBScienceData.argtypes = [
        ct.c_ulong, ct.c_ushort, ct.c_long, ct.POINTER(ct.c_ushort*n_science_words_read)
    ]
    MACIE.MACIE_ReadUSBScienceData.restype = ct.c_int
    science_words = (ct.c_ushort*n_science_words_read)()
    n_science_words_output = MACIE.MACIE_ReadUSBScienceData(
        handle, timeout, n_science_words_read, ct.byref(science_words)
    )
    return handle_image_array(science_words, n_science_words_output)
...
def close_camlink_science_interface(handle, select_macies):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_CloseCamlinkScienceInterface( unsigned long handle,
                                                                        unsigned char slctMACIEs);
    """
    MACIE.MACIE_CloseCamlinkScienceInterface.argtypes = [ct.c_ulong, ct.c_ubyte]
    MACIE.MACIE_CloseCamlinkScienceInterface.restype = Status
    handle_status(MACIE.MACIE_CloseCamlinkScienceInterface(handle, select_macies))
...
def close_gige_science_interface(handle, select_macies):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_CloseGigeScienceInterface( unsigned long handle,
                                                                     unsigned char slctMACIEs);
    """
    MACIE.MACIE_CloseGigeScienceInterface.argtypes = [ct.c_ulong, ct.c_ubyte]
    MACIE.MACIE_CloseGigeScienceInterface.restype = Status
    handle_status(MACIE.MACIE_CloseGigeScienceInterface(handle, select_macies))
...
def close_usb_science_interface(handle, select_macies):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_CloseUSBScienceInterface( unsigned long handle,
                                                                    unsigned char slctMACIEs);
    """
    MACIE.MACIE_CloseUSBScienceInterface.argtypes = [ct.c_ulong, ct.c_ubyte]
    MACIE.MACIE_CloseUSBScienceInterface.restype = Status
    handle_status(MACIE.MACIE_CloseUSBScienceInterface(handle, select_macies))
...
def set_voltage(handle, select_macies, power_id, power_value):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_SetVoltage( unsigned long handle,
                                                      ...
                                                      float         powerValue);
    """
    MACIE.MACIE_SetVoltage.argtypes = [ct.c_ulong, ct.c_ubyte, PowerDAC, ct.c_float]
    MACIE.MACIE_SetVoltage.restype = Status
    handle_status(MACIE.MACIE_SetVoltage(handle, select_macies, power_id, power_value))
...
def get_voltage(handle, select_macies, power_id):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_GetVoltage( unsigned long handle,
                                                      ...
                                                      float*        powerValue);
    """
    MACIE.MACIE_GetVoltage.argtypes = [ct.c_ulong, ct.c_ubyte, PowerDAC, ct.POINTER(ct.c_float)]
    MACIE.MACIE_GetVoltage.restype = Status
    power_value = ct.c_float(0)
    handle_status(MACIE.MACIE_GetVoltage(handle, select_macies, power_id, ct.byref(power_value)))
...
def enable_power(handle, select_macies, power_control_ids):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_EnablePower( unsigned long handle,
                                                       ...
                                                       short          n);
    """
    n_power_id = len(power_control_ids)
    MACIE.MACIE_EnablePower.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_int * n_power_id, ct.c_short]
    MACIE.MACIE_EnablePower.restype = Status
    power_control_id_array = (ct.c_int * n_power_id)(*power_control_ids)
    handle_status(
        MACIE.MACIE_EnablePower(handle, select_macies, power_control_id_array, n_power_id)
    )
...
def disable_power(handle, select_macies, power_control_ids):
    ...
    """
    n_power_id = len(power_control_ids)
    MACIE.MACIE_DisablePower.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_int * n_power_id, ct.c_short]
    MACIE.MACIE_DisablePower.restype = Status
    power_control_id_array = (ct.c_int * n_power_id)(*power_control_ids)
    handle_status(
        MACIE.MACIE_DisablePower(handle, select_macies, power_control_id_array, n_power_id)
    )
...
def set_power(handle, select_macies, power_control_id, power_state):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_SetPower( unsigned long handle,
                                                    ...
                                                    bool           bEnablePower);
    """
    MACIE.MACIE_SetPower.argtypes = [ct.c_ulong, ct.c_ubyte, PowerControl, ct.c_bool]
    MACIE.MACIE_SetPower.restype = Status
    MACIE.MACIE_SetPower(handle, select_macies, power_control_id, power_state)
...
def get_power(handle, select_macies, power_control_id):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_GetPower( unsigned long handle,
                                                    ...
                                                    bool           bEnablePower);
    """
    MACIE.MACIE_GetPower.argtypes = [ct.c_ulong, ct.c_ubyte, PowerControl, ct.POINTER(ct.c_bool)]
    MACIE.MACIE_GetPower.restype = Status
    power_state = ct.c_bool(False)
    MACIE.MACIE_GetPower(handle, select_macies, power_control_id, ct.byref(power_state))
...
def set_telemetry_configuration(
    ...
    """
    MACIE.MACIE_SetTelemetryConfiguration.argtypes = [
        ct.c_long, ct.c_ubyte, ct.c_int, ct.c_int, ct.c_int, ct.c_int, ct.c_int
    ]
    MACIE.MACIE_SetTelemetryConfiguration.restype = Status
    handle_status(
        MACIE.MACIE_SetTelemetryConfiguration(
            handle, select_macies, v_sample_rate, v_average, i_sample_rate, i_average, ground_reference
        )
    )
...
def get_telemetry_configuration(
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_GetTelemetryConfiguration( unsigned long handle,
                                                                     ...
                                                                     MACIE_TLM_GROUND_REFERENCE* groundRef);
     """
    MACIE.MACIE_GetTelemetryConfiguration.argtypes = [
        ct.c_long, ct.c_ubyte, ct.POINTER(ct.c_int), ct.POINTER(ct.c_int), ct.POINTER(ct.c_int), ct.POINTER(ct.c_int),
        ct.POINTER(ct.c_int)
    ]
    MACIE.MACIE_GetTelemetryConfiguration.restype = Status
    v_sample_rate = v_average = i_sample_rate = i_average = ground_reference = ct.c_int()
    handle_status(
        MACIE.MACIE_GetTelemetryConfiguration(
            handle, select_macies, ct.byref(v_sample_rate), ct.byref(v_average), ct.byref(i_sample_rate),
            ct.byref(i_average), ct.byref(ground_reference)
        )
    )
    return TLMSampleRate(v_sample_rate.value), TLMAverage(v_average.value), TLMSampleRate(i_sample_rate.value),\
        TLMAverage(i_average.value), TLMGroundReference(ground_reference.value)
...
def get_telemetry(handle, select_macies, tlm_id):
    ...
    MACIESHARED_EXPORT MACIE_STATUS MACIE_GetTelemetry( unsigned long handle,
                                                        ...
                                                        float*         tlmValue);
    """
    MACIE.MACIE_GetTelemetry.argtypes = [ct.c_ulong, ct.c_ubyte, ct.c_int, ct.POINTER(ct.c_float)]
    MACIE.MACIE_GetTelemetry.restype = Status
    tlm_value = ct.c_float(0)
    MACIE.MACIE_GetTelemetry(handle, select_macies, tlm_id, ct.byref(tlm_value))
    return tlm_value.value
...
def get_telemetry_set(handle, select_macies, tlm_ids):
    ...
    MACIE.MACIE_GetTelemetry.argtypes = [
        ct.c_ulong, ct.c_ubyte, ct.c_int * n_tlm_ids, ct.c_short, ct.POINTER(ct.c_float*n_tlm_ids)
    ]
    MACIE.MACIE_GetTelemetry.restype = Status
    tlm_values = (ct.c_float * n_tlm_ids)()
    tlm_id_array = (ct.c_int * n_tlm_ids)(*tlm_ids)
    MACIE.MACIE_GetTelemetry(handle, select_macies, tlm_id_array, n_tlm_ids, ct.byref(tlm_values))
    return [tlm_values[i] for i in range(n_tlm_ids)]
...
def get_telemetry_all(handle, select_macies):
    ...
    MACIE.MACIE_GetTelemetry.argtypes = [ct.c_ulong, ct.c_ubyte, ct.POINTER(ct.c_float * n_tlm_items)]
    MACIE.MACIE_GetTelemetry.restype = Status
    tlm_values = (ct.c_float * n_tlm_items)()
    MACIE.MACIE_GetTelemetry(handle, select_macies, ct.byref(tlm_values))
    return [tlm_values[i] for i in range(n_tlm_items)]
